...
describe.skipIf(testDatabaseUrl() === undefined)("grants, forget and purge", () => {
  ...
  test("legacy capture and find grants authorize neither add nor search", async () => {
    expect((await post("frank", "/add", { title: "t", text: "b" })).status).toBe(403);
  ...
  test("malformed bodies and out-of-range limits are 400", async () => {
    expect((await post("alice", "/add", { title: "no text" })).status).toBe(400);
    expect((await post("alice", "/search", { query: "q", limit: 51 })).status).toBe(400);
    expect((await post("alice", "/search", { query: "q", kinds: [1] })).status).toBe(400);
  });

  test("list and search show a document to its creator and to holders of a grant on its tags only", async () => {
    await post("alice", "/add", { title: "Alice private", text: "only alice" });
  ...
  test("purge is refused for a non-creator holding the purge grant", async () => {
    ...
    expect((await post("carol", `/documents/${documentId}/purge`)).status).toBe(403);
  });
  ...
  test("search passes kinds and entity_ids through, and empty arrays filter nothing", async () => {
    await post("alice", "/add", { title: "Mango decision", text: "Mango wins.", kind: "decision" });
    await post("alice", "/add", { title: "Mango note", text: "Mango is ripe." });
    ...
    expect(await titles({ query: "mango", kinds: ["decision"] })).toEqual(["Mango decision"]);
    expect(await titles({ query: "mango", kinds: [], entity_ids: [] })).toEqual([
      "Mango decision",
      "Mango note",
    ]);
    expect(await titles({ query: "mango", entity_ids: ["entity-none"] })).toEqual([]);
  });
  ...
  test("the forget grant does not authorize purge, and forget never deletes rows", async () => {
    ...
    expect((await post("erin", `/documents/${documentId}/purge`)).status).toBe(403);
  ...
  test("unknown documents and versions are 404, and whitespace ids are 400", async () => {
    expect((await post("alice", "/documents/doc-missing/forget")).status).toBe(404);
    expect((await post("alice", "/documents/doc-missing/purge")).status).toBe(404);
    expect(
      (await post("alice", "/versions/ver-missing/retention-class", { retention_class: "durable" }))
        .status,
    ).toBe(404);
    expect((await post("alice", "/documents/%20/forget")).status).toBe(400);
    expect((await post("alice", "/documents/%20/purge")).status).toBe(400);
    expect(
      (await post("alice", "/versions/%20/retention-class", { retention_class: "durable" }))
        .status,
    ).toBe(400);
  });
  ...
  test("retention-class changes a version for its creator only and validates the class", async () => {
    ...
    expect((await post("alice", path, { retention_class: "forever" })).status).toBe(400);
    expect((await post("carol", path, { retention_class: "durable" })).status).toBe(403);
    expect((await post("alice", path, { retention_class: "durable" })).status).toBe(200);
