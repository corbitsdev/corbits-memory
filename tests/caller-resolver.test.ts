...
describe.skipIf(testDatabaseUrl() === undefined)("machine callers through callerResolver", () => {
  let db: TestDb;
  ...
  test("add writes under the resolved scope, ignoring identity in the body", async () => {
    ...
    const [row] = await db.sql<{ tenant_id: string; created_by_principal_id: string }[]>`
      SELECT d.tenant_id, v.created_by_principal_id
        FROM memory.document d JOIN memory.version v ON v.document_id = d.id
        WHERE d.id = ${documentId}`;
    expect(row).toEqual({ tenant_id: "acme", created_by_principal_id: "run" });
  });
  ...
  test("the resolved caller still needs the grant on every route", async () => {
    ...
    expect((await post(ungranted, "/add", { title: "t", text: "b" })).status).toBe(403);
    expect((await post(ungranted, "/search", { query: "q" })).status).toBe(403);
    expect((await ungranted.request("/api/tenants/acme/memory/list")).status).toBe(403);
    expect((await ungranted.request("/api/tenants/acme/memory/feed")).status).toBe(403);
  });

  test("a resolver that cannot identify the caller is 401", async () => {
    const res = await post(appFor(() => null), "/add", { title: "t", text: "b" });
  ...
  ] as const)("a resolver returning a %s identity is a 500, never a seated scope", async (_label, resolved) => {
    // The malformed principal holds an add grant, so a 500 proves the
    // identity was rejected before the grant check.
    const before = await documentCount();
    const res = await post(appFor(() => resolved), "/add", { title: "t", text: "b" });
    expect(await documentCount()).toBe(before);
    expect(res.status).toBe(500);
    expect(((await res.json()) as { error: { code: string } }).error.code).toBe(
      "invalid_resolved_caller",
    );
  });
  ...
  test("forget, purge and retention-class work for the resolved creator only", async () => {
    ...
    expect((await post(other, `/documents/${forgotten}/forget`)).status).toBe(403);
    expect((await post(owner, `/documents/${forgotten}/forget`)).status).toBe(200);

    const purged = await add(owner, "Purge via run");
    expect((await post(owner, `/documents/${purged}/purge`)).status).toBe(200);
    ...
    expect((await post(other, path, { retention_class: "durable" })).status).toBe(403);
    expect((await post(owner, path, { retention_class: "durable" })).status).toBe(200);
  });
});
